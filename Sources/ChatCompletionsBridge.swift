...
enum ProviderConnectionTester {
    static func test(profile: ProviderProfile, key: String) async throws -> String {
        ...
        request.setValue("Agent-Pulse/2.7.0", forHTTPHeaderField: "User-Agent")
...
final class ChatCompletionsBridge {
    ...
    private let queue = DispatchQueue(label: "net.nexita.agent-pulse.chat-bridge", qos: .userInitiated)
    ...
    func start() throws {
        ...
        listener.stateUpdateHandler = { state in
            if case let .failed(error) = state {
                NSLog("Agent Pulse chat bridge failed: %@", error.localizedDescription)
            }
        }
    ...
    private func handle(_ request: HTTPRequest, connection: NWConnection) {
        ...
        do {
            ...
            upstream.setValue("Agent-Pulse/2.7.0", forHTTPHeaderField: "User-Agent")
